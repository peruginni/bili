  ...
  init(
    ...
    var body: some View {
        ZStack {
            ...
            .clipShape(
                RoundedRectangle(cornerRadius: .grid(self.config.idiom == .pad ? 4 : 10), style: .continuous)
            )
            .clipped()
            .padding(.grid(self.config.idiom == .pad ? 10 : 4))
            ...
            .scaleEffect(self.config.idiom == .pad ? 0.8 : 0.9)
            .offset(y: .grid(self.config.offset))
            
            VStack(spacing: .grid(self.config.idiom == .pad ? 14 : 7)) {
                VStack(spacing: .grid(self.config.idiom == .pad ? 14 : 7)) {
//                    Image(systemName: "cube.fill")
//                        .foregroundColor(Color.black)
//                        .font(.system(size: self.config.idiom == .pad ? 50 : 30))
                    
                    self.description()
                        .font(self.config.descriptionFont)
                        .fontWeight(.semibold)
...
//                )
            ...
            .padding(.horizontal, .grid(self.config.idiom == .pad ? 40 : 10))
            .padding(.vertical, .grid(self.config.idiom == .pad ? 12 : 4))
        }
